...
# Custom collate function for handling multiple labels
def custom_collate_fn(batch):
    """
    Custom collate function to handle multiple labels from FineGrainChangeLabelDataset
    """
    images = []
    fine_labels = []
    base_labels = []
    ...
    for item in batch:
        if len(item) == 3:  # Training mode: (image, fine_label, base_label)
            img, fine_label, base_label = item
            images.append(img)
            fine_labels.append(fine_label)
            base_labels.append(base_label)
        else:  # Test mode: only image
            images.append(item)
    
    images = torch.stack(images)
    
    if fine_labels:  # Training mode
        fine_labels = torch.tensor(fine_labels, dtype=torch.long)
        return images, fine_labels, base_labels
    else:  # Test mode
        return images
...
def train():
    ...
    # 라벨 정보 추출
    targets = [label for _, label, _ in [full_dataset[i] for i in range(len(full_dataset))]]
    ...
    for model_idx in range(2):  # 논문에 따라 정확히 2개 모델
        ...
        for epoch in range(config.epochs):
            ...
            for batch_data in tqdm(model_train_loader,
                                 desc=f"[Model {model_idx+1} Epoch {epoch+1}] Train"):
                images, fine_labels, base_labels = batch_data
            ...
            with torch.no_grad():
                for batch_data in tqdm(model_val_loader,
    ...
    for epoch in range(20):  # 논문에서는 fine-tuning이므로 적은 epoch 사용
        ...
        for batch_data in tqdm(full_train_loader,
            ...
            with torch.cuda.amp.autocast():
                ...
                loss = combination_criterion(final_output, fine_labels)
        ...
        with torch.no_grad():
            for batch_data in tqdm(full_val_loader,
